...
def test_config_change_invalidates_cache(reset_prefetch_state, monkeypatch, tmp_path):
    ...
    assert app_module._prefetch_cache['path'] is None, "_prefetch_cache['path'] should be None after config change"
...
def test_single_thread_guard(reset_prefetch_state, monkeypatch):
    ...
    assert not called, 'prefetch_next_image was called even though a live thread was already running'
    assert app_module._prefetch_thread is thread_before, '_prefetch_thread identity changed — a new thread was spawned'
